...
bool isCollision(int x, int y, SDL_Rect position) {
    // Debug information.
    DEBUG_PRINT(stdout, "Click: %d, %d Entity: %d-%d, %d-%d\n", x, y, position.x, position.x + position.w,
...
void clickHandler(GameData* gameData) {
    ...
    if (gameData->event.button.button == SDL_BUTTON_LEFT) {
        if (gameData->event.type == SDL_MOUSEMOTION) {
            DEBUG_PRINT(stdout, "Mouse left clicked and dragged!\n");
            // Being dragged
            for (int i = 0; i < gameData->scene.entityManager->current; i++) {
                Entity* e = &gameData->scene.entityManager->entities[i];
            ...
            for (int i = 0; i < gameData->scene.entityManager->current; i++) {
                ...
                if (isCollision(x, y, e->position)) {
                    ...
                    DEBUG_PRINT(stdout, "Entity left clicked!\n");
                }
            }
        }
